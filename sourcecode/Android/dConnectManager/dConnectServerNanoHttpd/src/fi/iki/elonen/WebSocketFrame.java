...
public class WebSocketFrame {
    ...
    public static byte[] text2Binary(String payload) throws CharacterCodingException {
    	ByteBuffer buffer = TEXT_ENCODER.encode(CharBuffer.wrap(payload));
    	// encondeメソッドでは文字列分ではなく処理に必要なだけbyteを持つので
    	// limitを見て正しいサイズ分のみ取り出す必要がある。
    	byte[] retval = new byte[buffer.limit()];
