...
- (NSString *)createFileForPath:(NSString *)path contents:(NSData *)contents
{
    ...
    NSError *error;
    [fileMgr createDirectoryAtURL:[url URLByDeletingLastPathComponent] withIntermediateDirectories:YES
...
+ (NSString *) searchExtensionForMimeType:(NSString *)mimeType
{
    ...
    NSArray *extensionArr = [ext2mimeDict allKeysForObject:mimeType];
    if (extensionArr.count > 0) {
        if (extensionArr.count != 1) {
            // 複数；最も短く、辞書順で手前になる拡張子が先頭になるようにソート。
            extensionArr = [extensionArr sortedArrayWithOptions:0 usingComparator:^NSComparisonResult(id obj1, id obj2) {
