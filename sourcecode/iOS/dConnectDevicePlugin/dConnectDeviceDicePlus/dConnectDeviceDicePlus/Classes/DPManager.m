...
//
///*!
// @brief バッテリの情報を通知するブロック一覧。
// */
//@property (nonatomic) NSMutableDictionary *batteryBlocks;
//@property (nonatomic) NSMutableDictionary *batteryChargingBlocks;
//
///*!
// @brief Rollの方法を通知するブロック一覧。
// */
//@property (nonatomic) NSMutableDictionary *rollBlocks;
//
///*!
// @brief 温度を通知するブロック一覧。
// */
//@property (nonatomic) NSMutableDictionary *temperatureBlocks;
//
//@property (nonatomic) NSMutableDictionary *orientationBlocks;
//@property (nonatomic) NSMutableDictionary *accelerationDatas;
//@property (nonatomic) NSMutableDictionary *orientationDatas;
...
+ (DPManager *)sharedManager {
    static DPManager *sharedDPManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedDPManager = [DPManager new];
    });
...
//                [delegate manager:self didFinishedScan:_diceList];
...
- (void)die:(DPDie *)die didRoll:(DPRoll *)roll error:(NSError *)error {    DPDicePlusData *dice = [self getDicePlusDataByDie:die];
...
- (void)die:(DPDie*)die didUpdateProximity:(DPProximity*)proximity error:(NSError*)error {
    ...
    if (block) {
        block(proximityMainValue, 0, 0, 0);
    }
}
